...
exports.customerReferralProgram = void 0;
...
exports.customerReferralProgram = functions.https.onCall(async (payload, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Faça login.');
    const db = admin.firestore();
    const companyId = String(payload?.companyId || '');
    if (!companyId || companyId.includes('/'))
        throw new functions.https.HttpsError('invalid-argument', 'Empresa inválida.');
    ...
    if (payload.action === 'register') {
        ...
        if (!validCnpj(cnpj))
            throw new functions.https.HttpsError('invalid-argument', 'Informe um CNPJ válido para a empresa indicada.');
        if (!['comercio', 'servicos'].includes(businessType))
            throw new functions.https.HttpsError('invalid-argument', 'Selecione comércio ou serviços.');
        if (cnpj === digits(company.data()?.document || company.data()?.cnpj))
            throw new functions.https.HttpsError('invalid-argument', 'Não é permitido indicar a própria empresa.');
        if (!name || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email === context.auth.token.email)
            throw new functions.https.HttpsError('invalid-argument', 'Informe o nome e e-mail de outro cliente.');
        const id = `cnpj-${cnpj}`;
        await db.runTransaction(async (tx) => {
            const ref = records.doc(id);
            if ((await tx.get(ref)).exists)
                throw new functions.https.HttpsError('already-exists', 'Este CNPJ já foi indicado.');
        ...
        if (!root)
            throw new functions.https.HttpsError('permission-denied', 'O fechamento deve ser confirmado pela Blu.');
        const targetId = String(payload.targetCompanyId || '').trim();
        const id = String(payload.id || '');
        if (!targetId || targetId.includes('/') || !id || id.includes('/') || targetId === companyId)
            throw new functions.https.HttpsError('invalid-argument', 'Empresa indicada inválida.');
        await db.runTransaction(async (tx) => {
            const ref = records.doc(id);
            ...
            if (!record.exists || record.data()?.companyId !== companyId || !target.exists)
                throw new functions.https.HttpsError('not-found', 'Indicação ou empresa não encontrada.');
            if (!record.data()?.cnpj || record.data()?.cnpj !== digits(target.data()?.document || target.data()?.cnpj))
                throw new functions.https.HttpsError('failed-precondition', 'O CNPJ da empresa contratante não corresponde ao CNPJ indicado.');
            if (record.data()?.status === 'closed')
                return;
            if (existing.exists)
                throw new functions.https.HttpsError('already-exists', 'Empresa já contabilizada em outra indicação.');
...
//# sourceMappingURL=customerReferrals.js.map