...
exports.serviceScheduling = functions.https.onCall(async (payload, context) => {
    ...
    if (action === 'get_foundation') {
        ...
        return { services: definitions.map((definition) => ({ ...definition, product: productsById.get(definition.productId) || null })), inventoryProducts: products.filter((item) => item.type !== 'service').map((item) => ({ id: item.id, name: item.name, unit: item.unit || 'un', stockQuantity: Number(item.stockQuantity || 0), active: item.active !== false })), categories, professionals, clients, appointments, resources, packages, commissions, settings: settings.exists ? { id: settings.id, ...settings.data() } : null };
    }
    const timestamp = now();
    if (action === 'save_service') {
        ...
        await db().runTransaction(async (tx) => {
            ...
            const product = { companyId, type: 'service', name, category: cleanText(input.category, 120) || 'Serviços', description: cleanText(input.description, 5000), notes: cleanText(input.description, 5000), features: Array.isArray(input.features) ? input.features.map((value) => cleanText(value, 160)).filter(Boolean).slice(0, 30) : [], salePriceCents: Math.max(0, Math.round(Number(input.priceCents || 0))), promotionalPriceCents: Math.max(0, Math.round(Number(input.promotionalPriceCents || 0))), images: Array.isArray(input.images) ? input.images.slice(0, 3) : [], unit: 'serv', publicationStatus: draft ? 'draft' : 'published', active: !draft, salesChannels: { bluStore: !draft && Boolean(input.publishOnEcommerce) }, updatedAt: timestamp, updatedBy: uid, createdAt: previous.data()?.createdAt || timestamp, createdBy: previous.data()?.createdBy || uid };
            const ingredients = Array.isArray(input.ingredients) ? input.ingredients.map((item) => ({ productId: cleanText(item.productId, 160), name: cleanText(item.name, 160), quantity: Math.max(0, Number(item.quantity || 0)), unit: cleanText(item.unit || 'un', 30) })).filter((item) => item.productId && item.quantity > 0).slice(0, 50) : [];
    ...
    if (action === 'complete_appointment') {
        ...
        await db().runTransaction(async (tx) => {
            ...
            if (!appointmentSnapshot.exists || appointment.companyId !== companyId)
                throw new functions.https.HttpsError('not-found', 'Agendamento não encontrado.');
            if (appointment.stockDeductedAt) {
                tx.set(appointmentRef, { status: 'completed', completedAt: appointment.completedAt || timestamp, updatedAt: timestamp, updatedBy: uid }, { merge: true });
                return;
            }
            const serviceRef = db().collection('serviceDefinitions').doc(cleanText(appointment.serviceId, 160));
            const serviceSnapshot = await tx.get(serviceRef);
            const ingredients = Array.isArray(serviceSnapshot.data()?.ingredients) ? serviceSnapshot.data().ingredients : [];
            const productSnapshots = await Promise.all(ingredients.map((item) => tx.get(db().collection('products').doc(cleanText(item.productId, 160)))));
            productSnapshots.forEach((snapshot, index) => { const ingredient = ingredients[index]; const current = Number(snapshot.data()?.stockQuantity || 0); const quantity = Number(ingredient.quantity || 0); if (!snapshot.exists || snapshot.data()?.companyId !== companyId)
                throw new functions.https.HttpsError('failed-precondition', `Insumo ${ingredient.name || 'não encontrado'} inválido.`); if (current < quantity)
