...
exports.completePublicSale = exports.managePointOfSalePaymentSettings = exports.checkPointOfSalePayment = exports.createPointOfSalePayment = exports.managePointOfSaleRegister = void 0;
...
const companyMembership = async (uid, companyId) => {
    if (!companyId)
        throw new functions.https.HttpsError('invalid-argument', 'Empresa do PDV não informada.');
    ...
    if (!isOwner && !active)
        throw new functions.https.HttpsError('permission-denied', 'Você não possui acesso a esta empresa.');
    return { companyId };
};
const registerSummary = async (register) => {
    ...
    const paymentTotalsCents = completed.reduce((totals, sale) => {
    ...
    const supplyCents = movementValues.filter((item) => item.type === 'supply').reduce((sum, item) => sum + Number(item.amountCents || 0), 0);
    const withdrawalCents = movementValues.filter((item) => item.type === 'withdrawal').reduce((sum, item) => sum + Number(item.amountCents || 0), 0);
    return {
        id: register.id,
        ...data,
        ...
        supplyCents,
        withdrawalCents,
        movements: movementValues.sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt))),
        expectedCashCents: Number(data.openingAmountCents || 0) + cashSalesCents + supplyCents - withdrawalCents,
    };
};
exports.managePointOfSaleRegister = functions.https.onCall(async (payload, context) => {
    if (!context.auth)
    ...
    if (action === 'status')
        return openRegister ? registerSummary(openRegister) : null;
    if (action === 'open') {
        if (requestedRegisterId) {
            const existing = await firestore.collection('pointOfSaleRegisters').doc(requestedRegisterId).get();
            if (existing.exists) {
                if (existing.data()?.companyId !== companyId || existing.data()?.operatorId !== context.auth.uid)
                    throw new functions.https.HttpsError('permission-denied', 'O identificador do caixa pertence a outra operação.');
                return registerSummary(existing);
            }
        }
        if (openRegister)
            throw new functions.https.HttpsError('already-exists', 'Este usuário já possui um caixa aberto nesta empresa.');
        const openingAmountCents = Number(payload?.openingAmountCents || 0);
        if (!Number.isSafeInteger(openingAmountCents) || openingAmountCents < 0)
            throw new functions.https.HttpsError('invalid-argument', 'O saldo inicial é inválido.');
    ...
    if (action === 'close') {
        ...
        if (!targetRegister)
            throw new functions.https.HttpsError('failed-precondition', 'Este usuário não possui caixa aberto.');
        if (targetRegister.data()?.companyId !== companyId || targetRegister.data()?.operatorId !== context.auth.uid)
            throw new functions.https.HttpsError('permission-denied', 'Este caixa pertence a outro operador ou empresa.');
        if (targetRegister.data()?.status === 'closed')
            return registerSummary(targetRegister);
        ...
        if (!Number.isSafeInteger(countedCashCents) || countedCashCents < 0)
            throw new functions.https.HttpsError('invalid-argument', 'Informe o valor contado no caixa.');
        const now = new Date().toISOString();
        ...
        await firestore.runTransaction(async (tx) => {
            ...
            if (!fresh.exists || fresh.data()?.status !== 'open' || fresh.data()?.operatorId !== context.auth.uid)
                throw new functions.https.HttpsError('failed-precondition', 'O caixa já foi fechado ou pertence a outro operador.');
    ...
    if (['supply', 'withdrawal'].includes(action)) {
        if (!openRegister)
            throw new functions.https.HttpsError('failed-precondition', 'Abra o caixa antes de registrar a movimentação.');
        const amountCents = Number(payload?.amountCents || 0);
        const reason = String(payload?.reason || '').trim().slice(0, 500);
        if (!Number.isSafeInteger(amountCents) || amountCents <= 0)
            throw new functions.https.HttpsError('invalid-argument', 'Informe um valor maior que zero.');
        if (!reason)
            throw new functions.https.HttpsError('invalid-argument', 'Informe o motivo da movimentação.');
        ...
        await firestore.runTransaction(async (tx) => {
            ...
            if (!fresh.exists || fresh.data()?.status !== 'open' || fresh.data()?.operatorId !== context.auth.uid)
                throw new functions.https.HttpsError('failed-precondition', 'O caixa não está disponível para este operador.');
            tx.set(ref, value);
    ...
    if (['temporary_exit', 'resume'].includes(action)) {
        if (!openRegister)
            throw new functions.https.HttpsError('failed-precondition', 'Nenhum caixa aberto foi encontrado.');
    ...
    if (action === 'history') {
        ...
        return values.sort((a, b) => String(b.openedAt).localeCompare(String(a.openedAt))).slice(0, 30);
    }
...
const escapeHtml = (value) => String(value || '')
    ...
    .replace(/"/g, '&quot;').replace(/'/g, '&#39;');
const pagarmeRequest = async (path, method, body) => {
    ...
    if (!config.enabled || !secretKey)
        throw new functions.https.HttpsError('failed-precondition', 'Configure e habilite o Pagar.me no Blu HQ.');
        ...
        const request = https.request({ hostname, path: `/core/v5${path}`, method, headers: {
            ...
            response.on('end', () => {
                ...
                if ((response.statusCode || 500) >= 400)
                    return reject(new functions.https.HttpsError('internal', `Pagar.me recusou o pagamento (HTTP ${response.statusCode}).`, parsed));
                resolve(parsed);
            });
        });
...
exports.createPointOfSalePayment = functions.https.onCall(async (payload, context) => {
    ...
    const orderItems = items.map((item, index) => {
        ...
        if (!productSnaps[index].exists || product?.companyId !== companyId)
...
exports.checkPointOfSalePayment = functions.https.onCall(async (payload, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Faça login para continuar.');
...
exports.managePointOfSalePaymentSettings = functions.https.onCall(async (payload, context) => {
    if (!context.auth)
        throw new functions.https.HttpsError('unauthenticated', 'Faça login para continuar.');
    const companyId = String(payload?.companyId || '');
    await companyMembership(context.auth.uid, companyId);
    ...
    const completedSaleId = await firestore.runTransaction(async (tx) => {
        ...
        if (paymentMethod === 'pix') {
            if (!paymentIntentRef || !paymentIntentSnap?.exists)
                throw new functions.https.HttpsError('failed-precondition', 'Gere e confirme o PIX antes de concluir a venda.');
            const intent = paymentIntentSnap.data() || {};
            if (intent.companyId !== member.companyId || intent.clientId !== clientId)
                throw new functions.https.HttpsError('permission-denied', 'Este PIX pertence a outra empresa ou cliente.');
            if (intent.status !== 'paid')
                throw new functions.https.HttpsError('failed-precondition', 'O pagamento PIX ainda não foi confirmado pelo Pagar.me.');
            if (Number(intent.amountCents || 0) !== net)
                throw new functions.https.HttpsError('failed-precondition', 'O valor confirmado no PIX é diferente do total atual da venda. Gere um novo QR Code.');
            if (intent.consumedAt && intent.saleId !== saleRef.id)
                throw new functions.https.HttpsError('already-exists', 'Este pagamento PIX já foi utilizado em outra venda.');
            paid = true;
        ...
        if (paymentMethod === 'cash' && (!Number.isSafeInteger(cashReceivedCents) || cashReceivedCents < net))
            throw new functions.https.HttpsError('invalid-argument', 'O valor recebido em dinheiro deve cobrir o total da venda.');
