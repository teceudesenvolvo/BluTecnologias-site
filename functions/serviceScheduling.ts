...
export const serviceScheduling = functions.https.onCall(async (payload, context) => {
  ...
  if (action === 'get_foundation') {
    ...
    return {services:definitions.map((definition: any) => ({...definition, product:productsById.get(definition.productId) || null})),categories,professionals,clients,appointments,resources,packages,commissions,settings:settings.exists ? {id:settings.id,...settings.data()} : null};
  }
