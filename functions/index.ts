...
export const mutateCollection=functions.https.onCall(async(payload,context)=>{if(!context.auth)throw new functions.https.HttpsError('unauthenticated','Faça login para continuar.');const membership=await financialMembership(context.auth.uid),action=String(payload?.action||''),id=payload?.id?String(payload.id):'',input=cleanFinancialValue(payload?.value||{})as Record<string,any>;if(!['create','update'].includes(action))throw new functions.https.HttpsError('invalid-argument','Ação inválida.');await requireFinancialPermission(membership,action==='create'?'createCollection':'editCollection');const ref=id?admin.firestore().collection('collections').doc(id):admin.firestore().collection('collections').doc(),gross=Number(input.originalAmountCents),discount=Number(input.discountCents||0),interest=Number(input.interestCents||0),fine=Number(input.fineCents||0),net=gross-discount+interest+fine;if(!String(input.description||'').trim()||!String(input.organizationName||'').trim()||!String(input.dueDate||'')||!Number.isSafeInteger(gross)||gross<=0||net<0)throw new functions.https.HttpsError('invalid-argument','Descrição, cliente, vencimento e valor são obrigatórios.');await admin.firestore().runTransaction(async tx=>{const snap=await tx.get(ref),before=snap.exists?snap.data()!:null;if(action==='update'&&(!snap.exists||before?.companyId!==membership.companyId))throw new functions.https.HttpsError('not-found','Cobrança não encontrada.');if(before?.status==='received')throw new functions.https.HttpsError('failed-precondition','Cobranças recebidas não podem ser editadas livremente.');const now=new Date().toISOString(),received=Number(before?.receivedAmountCents||0),after={...input,companyId:membership.companyId,number:String(input.number||`COB-${Date.now()}`),receivedAmountCents:received,balanceAmountCents:Math.max(0,net-received),originType:String(input.originType||'manual'),attachmentUrls:Array.isArray(input.attachmentUrls)?input.attachmentUrls:[],status:String(input.status||'draft'),createdAt:before?.createdAt||now,createdBy:before?.createdBy||context.auth!.uid,updatedAt:now,updatedBy:context.auth!.uid,version:Number(before?.version||0)+1};tx.set(ref,after,{merge:false});const event=admin.firestore().collection('collectionEvents').doc();tx.set(event,{companyId:membership.companyId,collectionId:ref.id,type:before?'updated':'created',description:before?'Cobrança atualizada':'Cobrança criada',eventDate:now,createdAt:now,createdBy:context.auth!.uid});tx.set(admin.firestore().collection('financialAuditLogs').doc(),{companyId:membership.companyId,action,entityType:'collection',entityId:ref.id,userId:context.auth!.uid,createdAt:now,before,after});});return{id:ref.id};});
