...
const companyRecipientProfile = (companyId: string, company: Record<string, any>) => ({
  companyId,
  provider: 'pagarme',
...
export const createPointOfSalePayment = functions.https.onCall(async (payload, context) => {
  ...
  if (!items.length || (!clientId && !anonymousCustomer)) throw new functions.https.HttpsError('invalid-argument', 'Informe itens e cliente ou use Consumidor final.');
  const firestore = admin.firestore();
  ...
  if (!companySnap.exists || (clientId && (!clientSnap?.exists || clientSnap.data()?.companyId !== companyId))) throw new functions.https.HttpsError('not-found', 'Empresa ou cliente não encontrado.');
  let gross = 0;
  ...
  const customer: Record<string, string> = { name: customerName, email: customerEmail };
  if (customerDocument) {
...
export const completePublicSale = functions.https.onCall(async (payload, context) => {
  ...
  if ((!clientId && !anonymousCustomer) || !key || !items.length) throw new functions.https.HttpsError('invalid-argument', 'Cliente, itens e chave da operação são obrigatórios.');
  const firestore = admin.firestore();
  ...
  const serviceDefinitions = serviceSale
    ...
    : null;
  const definitionsByProduct = new Map(
    (serviceDefinitions?.docs || []).map(snapshot => [String(snapshot.data().productId || ''), {id:snapshot.id,...snapshot.data()}]),
  );
  const ingredientRequirements = new Map<string, {name:string;unit:string;quantity:number;serviceIds:Set<string>}>();
  if (serviceSale) items.forEach((item:any) => {
    const definition:any = definitionsByProduct.get(String(item.productId || ''));
    if (!definition) throw new functions.https.HttpsError('failed-precondition', 'Um serviço da venda não possui configuração operacional.');
    const multiplier = Number(item.quantityMilli || 0) / 1000;
    (Array.isArray(definition.ingredients) ? definition.ingredients : []).forEach((ingredient:any) => {
      const productId = String(ingredient.productId || '');
      if (!productId) return;
      const current = ingredientRequirements.get(productId) || {name:String(ingredient.name || ''),unit:String(ingredient.unit || 'un'),quantity:0,serviceIds:new Set<string>()};
  ...
  const completedSaleId = await firestore.runTransaction(async tx => {
    ...
    if (clientSnap && (!clientSnap.exists || clientSnap.data()?.companyId !== member.companyId)) throw new functions.https.HttpsError('not-found', 'Cliente ou órgão não encontrado.');
    ...
    const normalized = items.map((item: any, index: number) => {
      ...
      if (serviceSale && product?.type !== 'service') throw new functions.https.HttpsError('invalid-argument', 'O PDV de Serviços aceita somente itens do catálogo de serviços.');
      if (!serviceSale && product?.type === 'service') throw new functions.https.HttpsError('invalid-argument', 'Use o PDV de Serviços para vender este item.');
    ...
    if (serviceSale) ingredientSnaps.forEach((snapshot,index) => {
      const [productId, requirement] = ingredientEntries[index];
      const product = snapshot.data();
      if (!snapshot.exists || product?.companyId !== member.companyId) throw new functions.https.HttpsError('failed-precondition', `Insumo ${requirement.name || productId} não encontrado.`);
      const current = Number(product.stockQuantity || 0);
      if (current < requirement.quantity) throw new functions.https.HttpsError('failed-precondition', `Estoque insuficiente de ${requirement.name || product.name}. Disponível: ${current} ${requirement.unit}.`);
