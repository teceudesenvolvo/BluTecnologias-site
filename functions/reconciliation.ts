...
const getMembership = async (uid: string) => {
...
export const importBankStatement = functions.https.onCall(async (payload, context) => {
...
export const commandReconciliation = functions.https.onCall(async (payload, context) => {
  if (!context.auth) throw new functions.https.HttpsError('unauthenticated', 'Faça login para continuar.');
  ...
  const existing = await admin.firestore().collection('reconciliationLinks')
    ...
    .get();
  const requestedIds: string[] = Array.isArray(payload?.transactionIds)
  ...
  await admin.firestore().runTransaction(async (transaction) => {
    ...
    if (action === 'ignore') {
      ...
      if (!reason) throw new functions.https.HttpsError('invalid-argument', 'Itens ignorados exigem justificativa.');
      ...
      let amounts = transactionSnapshots.map((snapshot, index) => Number(
        payload?.amounts?.[index]
      ...
      if (!refs.length) throw new functions.https.HttpsError('invalid-argument', 'Selecione ao menos um lançamento.');
