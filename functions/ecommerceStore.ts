...
export const ecommerceStore = functions.https.onCall(async (payload, context) => {
  ...
  if (action === 'book_service') {
    ...
    if (!definition || !product.exists || product.data()?.type !== 'service' || product.data()?.salesChannels?.bluStore !== true || !service.onlineBookingEnabled) throw new functions.https.HttpsError('failed-precondition', 'Este serviço não aceita agendamento online.');
    const date = String(payload?.date || ''); const startTime = String(payload?.startTime || '');
    const name = String(payload?.name || '').trim(); const email = String(payload?.email || '').trim().toLowerCase();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}$/.test(startTime) || !name || !/^\S+@\S+\.\S+$/.test(email)) throw new functions.https.HttpsError('invalid-argument', 'Preencha nome, e-mail, data e horário.');
