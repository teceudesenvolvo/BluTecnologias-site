...
import { ArrowRight, Store, CalendarDays, Package, WalletCards, Users, ShoppingCart, Clock3, Layers } from 'lucide-react';
...
export const BusinessLanding: React.FC<{ type: keyof typeof content }> = ({ type }) => {
  ...
  return <div className="bg-white text-slate-950">
    <header className="sticky top-0 z-30 border-b border-slate-100 bg-white/95 backdrop-blur-xl"><nav className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-5 py-4" aria-label="Navegação comercial"><Link to="/empresas" aria-label="Blu para empresas"><img src={Logo} alt="Blu" className="h-10 w-24 object-contain"/></Link><div className="hidden items-center gap-7 text-sm font-semibold md:flex"><a href="#recursos">Soluções</a><Link to={plans}>Planos</Link><Link to={`/empresas/${type === 'comercio' ? 'servicos' : 'comercio'}`}>Para {type === 'comercio' ? 'serviços' : 'comércio'}</Link><Link to="/admin/login">Entrar</Link></div><Link to={plans} className="rounded-xl bg-slate-950 px-4 py-3 text-xs font-bold text-white sm:text-sm">Conhecer planos <ArrowRight className="ml-1 inline" size={15}/></Link></nav></header>
