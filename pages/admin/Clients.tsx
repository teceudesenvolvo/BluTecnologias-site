...
import { contactService, clientService, prospectService, certificateService, ContactLead, Prospect, ProspectFile, Certificate } from '../../services/firebase';
...
export const Clients: React.FC = () => {
  ...
  const handleAddSubItem = async (e: React.FormEvent) => {
    ...
    const newItem = { ...subItemForm, id: Date.now().toString() };
  ...
  const handleSendBilling = async (e: React.FormEvent) => {
    ...
    // Simula o envio e salva como invoice
    const newInvoice = {
    ...
    await clientService.update(managingClient.id, { invoices: newInvoicesList });
    await clientService.sendBilling(managingClient.id, billingForm);

    alert('Cobrança enviada com sucesso!');
    setManageTab('invoices');
    setManagingClient({ ...managingClient, invoices: newInvoicesList });
    setSaving(false);
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                <h3 className="text-xl font-bold text-slate-800">{managingClient.name}</h3>
                 ...
                 <form onSubmit={handleSendBilling} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <select className="px-4 py-2 rounded-lg border border-slate-200 bg-white" value={billingForm.bankAccount} onChange={e => setBillingForm({...billingForm, bankAccount: e.target.value})}>
                        ...
                        <option value="bb">Banco do Brasil - Ag 1234 CC 56789</option>
                        <option value="itau">Itaú - Ag 4321 CC 98765</option>
                      </select>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {certificates.filter(cert => {
                          ...
                          <label key={cert.id} className="flex items-center gap-2 bg-slate-50 px-3 py-2 rounded-lg border border-slate-200 cursor-pointer hover:bg-slate-100">
                            <input type="checkbox" className="rounded text-blue-600" onChange={e => {
                              const selected = billingForm.selectedCertificates;
                              if (e.target.checked) setBillingForm({...billingForm, selectedCertificates: [...selected, cert.id]});
                              else setBillingForm({...billingForm, selectedCertificates: selected.filter(id => id !== cert.id)});
                            }} />
