...
const steps: TourStep[] = [
  ...
  { selector: '[data-tour="navigation"]', title: 'Todos os módulos ficam aqui', description: 'Use o menu lateral para acessar vendas, serviços, financeiro, documentos, relatórios e configurações.' },
  { selector: '[data-tour="company"]', title: 'Empresa atual', description: 'Quando seu plano permitir mais empresas, troque o contexto por aqui. Todos os dados acompanham a empresa selecionada.' },
  { selector: '[data-tour="search"]', title: 'Encontre qualquer recurso', description: 'Pesquise páginas e funcionalidades sem precisar percorrer todo o menu.' },
...
export const FirstAccessTour: React.FC<{ userId: string; enabled: boolean }> = ({ userId, enabled }) => {
  ...
  const top = rect && rect.bottom + 16 + 260 < window.innerHeight ? rect.bottom + 16 : Math.max(16, (rect?.top || window.innerHeight / 2) - 276);
  ...
  return (
    <div className="fixed inset-0 z-[9998]" role="dialog" aria-modal="true" aria-label="Tour de primeiro acesso">
      ...
      <section className="fixed w-[calc(100vw-32px)] max-w-sm rounded-3xl border border-white/70 bg-white p-5 text-slate-900 shadow-2xl dark:border-white/10 dark:bg-slate-900 dark:text-white" style={{ left, top }}>
        ...
        <p className="mt-5 text-[11px] font-black uppercase tracking-[.18em] text-sky-600">Primeiros passos · {index + 1} de {steps.length}</p>
        ...
        <div className="mt-5 flex items-center gap-1.5">{steps.map((_, itemIndex) => <span key={itemIndex} className={`h-1.5 rounded-full transition-all ${itemIndex === index ? 'w-7 bg-sky-500' : 'w-1.5 bg-slate-200 dark:bg-white/15'}`} />)}</div>
        <div className="mt-5 flex items-center justify-between gap-3">
          ...
          <button onClick={() => index === steps.length - 1 ? complete() : setIndex(index + 1)} className="flex items-center gap-2 rounded-xl bg-sky-600 px-4 py-2.5 text-sm font-black text-white shadow-lg shadow-sky-600/20">{index === steps.length - 1 ? <><Check size={16} /> Concluir</> : <>Próximo <ArrowRight size={16} /></>}</button>
        </div>
      </section>
    </div>
  );
