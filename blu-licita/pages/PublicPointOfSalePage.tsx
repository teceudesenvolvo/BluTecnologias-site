...
export const PublicPointOfSalePage: React.FC<{ mode?: "commerce" | "services" }> = ({ mode = "commerce" }) => {
  ...
  const complete = async () => {
    ...
    try {
      ...
      setLastSale(localSale); setReceiptModal(true); setSales((current) => [localSale, ...current.filter((sale) => sale.id !== saleId)].slice(0, 20));
  ...
  const createPixPayment = async () => {
    if (clientMode === "anonymous") return setMessage("Para gerar PIX integrado, selecione um cliente cadastrado com documento e e-mail.");
