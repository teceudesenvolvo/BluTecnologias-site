...
import { AccountantInviteModal } from '../components/AccountantInviteModal';

export const TeamPage: React.FC = () => {
  const [members, setMembers] = React.useState<TeamMember[]>([]);
  const [loading, setLoading] = React.useState(true);
  const [open, setOpen] = React.useState(false);
  const [accountantOpen, setAccountantOpen] = React.useState(false);
  ...
  // Contadores são profissionais externos gratuitos e não consomem assento do plano.
  const activeSeats = Math.max(1, members.filter((member) => member.status !== 'inactive' && member.role.toLowerCase() !== 'contador').length || 1);
  ...
  return <div className="mx-auto max-w-[1400px] space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-blue-600">Pessoas e acesso</p><h1 className="mt-2 text-3xl font-bold">Equipe</h1><p className="mt-1 text-sm text-slate-500">Convide membros e conceda acessos independentes por empresa.</p><p className="mt-2 text-xs font-semibold text-slate-400">Uso do plano: {activeSeats}/{plan.label('users')} usuário(s)</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setAccountantOpen(true)} className="flex items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm font-bold text-blue-700"><UserPlus size={17}/>Adicionar contador</button><button onClick={openInvite} disabled={!canInvite} className="flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><UserPlus size={17}/>Inserir membro</button></div></header>
    ...
    {accountantOpen && (
      <AccountantInviteModal onClose={() => setAccountantOpen(false)} onSaved={load}/>
    )}
