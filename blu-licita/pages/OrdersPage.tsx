...
export const OrdersPage: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-5">
      ...
      {open && <OrderForm clients={clients} close={() => setOpen(false)} save={async (value) => { if (!user) return; await createCompanyDoc("serviceOrders", user.companyId, user.id, value); setOpen(false); await load(); }} />}
    </div>
  );
};

const OrderForm = ({ clients, close, save }: { clients: ContactLead[]; close: () => void; save: (value: Omit<Order, "id">) => Promise<void> }) => {
  const [form, setForm] = React.useState<Omit<Order, "id">>({ number: `OS-${new Date().getFullYear()}-`, kind: "service", clientId: "", clientName: "", contractId: "", contractName: "", description: "", amountCents: 0, status: "issued", dueDate: today() });
  const client = clients.find((item) => item.id === form.clientId);
  const contracts = ((client as any)?.contracts || []).filter(Boolean);
  return <div className="fixed inset-0 z-[130] grid place-items-center bg-slate-950/55 p-4"><section className="w-full max-w-3xl rounded-2xl bg-white shadow-2xl"><header className="flex items-center justify-between border-b p-5"><h2 className="font-bold">Nova ordem</h2><button onClick={close}><X/></button></header><form onSubmit={(event) => { event.preventDefault(); save(form); }} className="grid gap-4 p-5 md:grid-cols-2">
    ...
    <Field label="Valor (R$)" type="number" value={String(form.amountCents / 100)} set={(v) => setForm({ ...form, amountCents: Math.round(Number(v) * 100) })}/>
