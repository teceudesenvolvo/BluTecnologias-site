...
export const MemberSignupPage: React.FC = () => {
  ...
  React.useEffect(() => { if (invitedEmail) setForm((current) => ({ ...current, email: invitedEmail })); }, [invitedEmail]);
  return <main className="grid min-h-screen place-items-center bg-slate-50 p-4"><form onSubmit={async (event) => { event.preventDefault(); if (form.password !== form.confirm) { setError('As senhas não conferem.'); return; } setLoading(true); setError(''); try { if (scope === 'platform') { await platformTeamService.accept(token, form.name, form.email, form.password); navigate('/admin/hq'); } else { await teamService.accept(token, form.name, form.email, form.password); navigate('/admin/login'); } } catch (reason: any) { setError(reason.message || 'Não foi possível aceitar o convite.'); } finally { setLoading(false); } }} className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-7 shadow-xl">
