...
export interface InterestArea { id:string; name:string; keywords:string[]; active:boolean; states?:string[] }
...
const migrate=(value:unknown):InterestArea[]=>{
  if(Array.isArray(value)&&value.every((item)=>typeof item==='string'))return (value as string[]).map((keyword,index)=>({id:`legacy-${index}-${normalize(keyword).replace(/ /g,'-')}`,name:keyword,keywords:[keyword],active:true,states:[]}));
  if(Array.isArray(value))return (value as InterestArea[]).filter((area)=>area&&area.name).map((area,index)=>({id:area.id||`area-${index}`,name:area.name,keywords:unique([area.name,...(area.keywords||[])]),active:area.active!==false,states:[...new Set((area.states||[]).map((state)=>String(state).toUpperCase()).filter(Boolean))]}));
  return [];
};
...
export const interestSettingsService={
  ...
  async getStates(companyId:string):Promise<string[]>{const areas=await this.getAreas(companyId);return [...new Set(areas.filter((area)=>area.active).flatMap((area)=>area.states||[]))]},
